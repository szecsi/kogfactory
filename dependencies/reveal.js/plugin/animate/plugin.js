...
//console.warn(element, config);
	...
	function getAnimatedSVG( container ) {
		//while(container.getAttribute("waitForMath") == "waiting"){
		//	
		//}
		...
		var svg = elements.toArray().find(element => element.node.parentElement == container
		 || element.node.parentElement.parentElement == container
			);
		if(svg){
		...
		return svg;
		//return svgElement && svgElement.instance;
	}
...
//console.warn("setupAnimations");
		...
		if(!container.svg){
			console.log("deferring animation");
			container.reanimateNewMath = () => { setupAnimations( container, config ); };
			return;
		}
		delete container.reanimateNewMath;
